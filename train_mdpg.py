...
def compute_u(policy, optimizer, prev_u, isw, prev_g):
...
def main():
	...
	assert num_agents > 1
	setup = 'decentralized'
	fpath = os.path.join('mdpg_results', setup, args.env, str(dimension) + 'D', args.opt)
	...
	for episode in range(num_episodes):
		...
		for policy, optimizer, prev_u, isw, prev_g in zip(agents, optimizers, prev_u_list, isw_list, list_grad_traj_prev_weights):
			grad_surrogate = compute_u(policy, optimizer, prev_u, isw, prev_g)

		# take consensus of parameters
		agents = global_average(agents, num_agents)

		# need to modify update_weights with grad surrogate
		for policy, optimizer in zip(agents, optimizers):
			update_weights(policy, optimizer)
