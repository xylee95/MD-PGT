...
for i in range(num_agents):
	agents.append(Policy(action_dim=dimension))
...
def main():
	...
	for i in range(num_episodes):
		...
		for t in range(1, max_eps_len):  # Don't infinite loop while learning
			...
			actions = np.clip(actions, env.min_action, env.max_action)
	...
	plt.ylabel('Reward')
	plt.xlabel('Episodes')
	plt.title(str(dimension) + '-d D_Rastrigin')
	plt.savefig(str(dimension) + '-d D_rastrigin_R.jpg')
	plt.close()
	...
	plt.xlabel('Episodes')
	plt.title(str(dimension) + '-d D_Rastrigin')
	plt.savefig(str(dimension) + '-d D_rastrigin_y.jpg')
