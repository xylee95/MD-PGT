...
def main():
	...
	if args.opt == 'Adam':
		...
		for i in range(num_agents):
			...
			optimizers.append(gt_opt.SGD(agents[i].parameters(), lr=3e-4))
	...
	plt.savefig(str(dimension) + '-d ' + setup + ' ' + args.env + '_' + args.opt + '_R_GT.jpg')
	plt.close()
	...
	plt.xlabel('Episodes')
	plt.title(str(dimension) + '-d ' + setup + ' ' + args.env)
	plt.savefig(str(dimension) + '-d ' + setup + ' ' + args.env + '_' + args.opt + '_Y_GT.jpg')
